P4Level = Class{__includes = BaseState}

function P4Level:render()
    push:apply('start')
    local wb = gTextures['p4Background']:getWidth()
    local hb = gTextures['p4Background']:getHeight()
    love.graphics.draw(gTextures['p4Background'],0,0,0,VIRTUAL_WIDTH / (wb - 1), VIRTUAL_HEIGHT / (hb - 1))
    love.drawBack()
...
function P4Level:mousePressed(x,y)
