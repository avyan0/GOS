...
function drawSettingsPanel(x, y, w)
    ...
    row('Master volume', data.volume, y + 98, function(v) if v ~= data.volume then love.audio.setVolume(v / 100) end data.volume = v end, ui.c.accent)
    row('Sound effects', data.sfx, y + 172, function(v) data.sfx = v end, ui.c.good)
end
...
function Settings:render()
    ...
    ui.panel(px, py, pw, 250, {radius = 16})
    ...
    ui.text('Click a weapon or press A / S / D   -   click a lane or tile to aim   -   Enter ends the turn   -   P pauses', px + 30, py + 140, pw - 60, 'left', 'body', 15)
    ui.text('Progress is saved automatically.', px + 30, py + 200, pw - 60, 'left', 'body', 15, ui.c.muted)

    ui.panel(40, 380, 1200, 110, {radius = 16})
    ui.text('Gods of Space   -   made by Avyan Mahajan', 70, 405, 800, 'left', 'body', 16, ui.c.muted)
    ui.text('Built with LÖVE', 70, 430, 800, 'left', 'body', 14, ui.c.dim)
    if ui.button('Quit game', 1000, 405, 210, 50, {outline = true, color = ui.c.danger, size = 20}) then love.event.quit() end
end
