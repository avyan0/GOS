...
function weaponDictionary()
    ...
    add(makeWeapon('ElectroJolt', 'rare', 'Electro Jolt', 'jolt', 'Stuns every alien in a lane for two turns.', {aoe = 'lane', cd = 2, stun = 10, stunTurns = 2}))
    ...
    add(makeWeapon('Respawn', 'scarce', 'Lockdown', 'block', 'Nothing can spawn in the lane you choose for the next two turns. Hevalten ignore it.', {aoe = 'lane', cd = 2}))
    add(makeWeapon('Offguard', 'scarce', 'Off Guard', 'freeze', 'Stuns every alien on the field for two turns.', {aoe = 'all', cd = 3, stun = 10, stunTurns = 2}))
    ...
    add(makeWeapon('DeathVirus', 'god', 'Death Virus', 'skull', 'Wipes out every alien in a random occupied lane.', {aoe = 'Random Lane', cd = 3}))
...
function alienDictionary()
    ...
    add(makeAlien('Joe',              750,   false, 'Joe',            'Just a guy. No ability.',                                                          {shape = 'round',   eyes = 2, hue = 0.33}))
    add(makeAlien('Gen57',            1050,   false, 'Gen-57',         'Mass-produced clone. No ability.',                                                 {shape = 'square',  eyes = 2, hue = 0.55}))
    add(makeAlien('President',        1650,  false, 'President',      'Elected, somehow. No ability.',                                                    {shape = 'round',   eyes = 2, hue = 0.62}))
    add(makeAlien('King',             2300,  false, 'King',           'Rules by birthright. No ability.',                                                 {shape = 'hex',     eyes = 2, hue = 0.12, crown = true}))
    add(makeAlien('DJ',               2400,  false, 'DJ',             'Loud. No ability.',                                                                {shape = 'round',   eyes = 1, hue = 0.85}))
    add(makeAlien('SpaceFence',       2550,  false, 'Space Fence',    'Spawns with a one-turn shield that blocks all damage.',                            {shape = 'square',  eyes = 3, hue = 0.5}))
    add(makeAlien('Spaceship',        2500,  false, 'Spaceship',      'Takes flight every second turn. While flying it is immune to almost everything.', {shape = 'diamond', eyes = 1, hue = 0.58}))
    add(makeAlien('VRWorkout',        2800,  false, 'VR Workout',     'Very fit. No ability.',                                                            {shape = 'round',   eyes = 2, hue = 0.05}))
    add(makeAlien('OldGranny',        2950,  false, 'Old Granny',     'If she was hurt this turn, lurches an extra tile forward before everyone moves.',                           {shape = 'round',   eyes = 2, hue = 0.9,  sat = 0.3}))
    add(makeAlien('Albot',            2300,  true,  'Albot',          'Hevalten. Spawns a random alien in its row every turn.',                          {shape = 'square',  eyes = 3, hue = 0.0}))
    add(makeAlien('Jumper',           5100,  false, 'Jumper',         'Leaps over any wall in its way.',                                                  {shape = 'tri',     eyes = 2, hue = 0.28}))
    add(makeAlien('Giant',            9400,  false, 'Giant',          'Enormous, but only moves every other turn.',                                       {shape = 'hex',     eyes = 2, hue = 0.08, sat = 0.5}))
    add(makeAlien('Gardener',         7600,  false, 'Gardener',       'Walls cannot be placed in any lane the Gardener occupies.',                        {shape = 'round',   eyes = 2, hue = 0.25}))
    add(makeAlien('Army',             8500,   true,  'Army',           'Hevalten. Every Army on the field lowers all of your damage by 2%.',              {shape = 'square',  eyes = 2, hue = 0.2,  sat = 0.4}))
    add(makeAlien('Morpher',          5800,  false, 'Morpher',        'Turns into a different alien every turn, keeping its health percentage.',          {shape = 'diamond', eyes = 3, hue = 0.75}))
    add(makeAlien('Fusion',           7600,  false, 'Fusion',         'On spawn, fuses two aliens into one with 1.5x their combined health.',             {shape = 'hex',     eyes = 1, hue = 0.95}))
    add(makeAlien('CommonCrippler',   10200,  false, 'Crippler',       'Common weapons deal 25% less damage while it lives.',                             {shape = 'tri',     eyes = 2, hue = 0.4}))
    add(makeAlien('Splashfest',       11000,  false, 'Splashfest',     'Immune to all splash damage.',                                                     {shape = 'round',   eyes = 3, hue = 0.52}))
    add(makeAlien('Virus',            14500,  false, 'Virus',          'Every weapon that has a cooldown takes one turn longer to recharge.',                               {shape = 'diamond', eyes = 1, hue = 0.3}))
    add(makeAlien('Guardian',         25000, true,  'Guardian',       'Hevalten. Absorbs all damage aimed at its lane. Immune to poison, knockback and hypnosis.', {shape = 'hex', eyes = 2, hue = 0.6, aura = true}))
    add(makeAlien('DarkArts',         17300,  true,  'Dark Arts',      'Hevalten. On spawn, upgrades the three aliens closest to your base into stronger ones.',        {shape = 'diamond', eyes = 2, hue = 0.78, sat = 0.9}))
    add(makeAlien('Rare',             20200,  false, 'Rarebane',       'Rare weapons deal 20% less damage while it lives.',                               {shape = 'square',  eyes = 1, hue = 0.6}))
    add(makeAlien('Protected',        22100, false, 'Bunker',         'All targeted damage against it is halved.',                                        {shape = 'square',  eyes = 2, hue = 0.1,  sat = 0.3}))
    add(makeAlien('Interdimentional', 27800, false, 'Interdimensional', 'Every one on the field reduces all damage by 7.5%.',                            {shape = 'diamond', eyes = 3, hue = 0.7}))
    add(makeAlien('Scarce',           53500, true,  'Scarcebane',     'Hevalten. Scarce weapons deal 20% less damage while it lives.',                   {shape = 'hex',     eyes = 1, hue = 0.8}))
    add(makeAlien('TheHevalGod',      68000, true,  'Heval God',      'Hevalten. If it was hurt this turn, calls a random Hevalten into the first three rows.', {shape = 'hex', eyes = 3, hue = 0.0, aura = true, crown = true}))
    add(makeAlien('GodOfSpace',       74500, true,  'God of Space',   'Hevalten. Spawns three more aliens on arrival. Immune to knockback, poison and hypnosis.', {shape = 'round', eyes = 3, hue = 0.15, aura = true, crown = true}))
    -- newer arrivals
    add(makeAlien('Swarmling',        600,   false, 'Swarmling',      'Tiny and fast: moves two rows every turn.',                                        {shape = 'tri',     eyes = 1, hue = 0.45}))
    add(makeAlien('Shieldbearer',     2650,  false, 'Shieldbearer',   'Each turn, shields the alien directly ahead of it for one turn.',                  {shape = 'square',  eyes = 2, hue = 0.13}))
    add(makeAlien('Phaser',           3200,  false, 'Phaser',         'Half damage from lane attacks. Double damage from tile and single-target attacks.', {shape = 'diamond', eyes = 2, hue = 0.5, sat = 0.9}))
    add(makeAlien('Medic',            6100,  false, 'Medic',          'Each turn, heals every other alien for 10% of its maximum health.',                 {shape = 'round',   eyes = 2, hue = 0.0, sat = 0.2}))
    add(makeAlien('Thief',            6800,  false, 'Thief',          'Steals 5 gold from you every turn it is alive.',                                     {shape = 'diamond', eyes = 2, hue = 0.16, sat = 0.9}))
    add(makeAlien('Splitter',         8500,  false, 'Splitter',       'When it dies, breaks into two Swarmlings that each carry a quarter of its health.',                       {shape = 'hex',     eyes = 4, hue = 0.35}))
    add(makeAlien('Anchor',           13600,  false, 'Anchor',         'Nothing in its lane can be knocked back, pulled or moved by your weapons.',          {shape = 'square',  eyes = 1, hue = 0.6, sat = 0.3}))
    add(makeAlien('Necromancer',      18200,  true,  'Necromancer',    'Hevalten. Every third turn, raises the last alien you killed at half health in its row.', {shape = 'diamond', eyes = 3, hue = 0.82, aura = true}))
    add(makeAlien('VoidTitan',        84000, true,  'Void Titan',     'Hevalten. No single hit can deal more than 5000 damage to it.',                     {shape = 'hex',     eyes = 3, hue = 0.68, aura = true, crown = true}))
